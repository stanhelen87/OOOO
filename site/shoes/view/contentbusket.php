...
<div class="right">
    ...
    <ul>
        <?php
        //Запоминаем id_inkind в сессиии для последующего возврата
        // $_SESSION['product']=$data;
        $m=count($kindparent)-1;
        for ($i = 0; $i <= $m; $i++)       {
        ?>
        <li ><a  href="#"><?php echo $kindparent[$i] ?> </a>
            <ul class="nav nav-pills nav-stacked" ><?php foreach ($data1 as $record) {
                    if ($record['kindparent'] == $kindparent[$i]) { /* echo $record['id_inkind']*/ ?>
                        <li><a href="<?php echo SITEBASE.'shoes/product/select/id_inkind/'.$record['id_inkind']?>"><?php echo $record['inkind'];?> </a></li>
